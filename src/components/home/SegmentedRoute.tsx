...
export default function SegmentedRoute() {
  ...
  const segmentedRouteFinder = useMatrixStore(
    (state) => state.segmentedRouteFinder
  );
  const segmentedSeatArray = useMatrixStore(
    (state) => state.segmentedSeatArray
  );
  const segmentedRouteFound = useMatrixStore(
  ...
  return (
    <div className="segmented-route w-66 sm:w-96 sm:grid-cols-2 grid p-4  justify-items-center  rounded-md  gap-4 mt-4 mb-4">
      <h1 className="text-blue-600 font-bold text-xl text-center sm:col-span-2">
      ...
      <Button
        color="primary"
        className="p-5 w-40 h-18 font-bold sm:col-span-2"
        onPress={() => segmentedRouteFinder()}
      >
        Check Availability
      </Button>
