...
export default function SegmentedDestinationStationDropdown() {
  ...
  const setSegmentedDestinationStation = useMatrixStore(
    (state) => state.setSegmentedDestinationStation
  );
    ...
    <div className="sm:col-span-2">
      <Autocomplete label="Destination Station">
        {formattedRouteList.map((item) => (
          <AutocompleteItem
            key={item.key}
            onPress={() => setSegmentedDestinationStation(item.label as string)}
          >
            {item.label}
          </AutocompleteItem>
        ))}
